...
function req(query = `?org_id=${validOrg}`) {
  return new NextRequest(new URL(`/api/connect/places${query}`, "http://localhost"));
}
...
describe("GET /api/connect/places", () => {
  ...
  it("returns the org's places for members", async () => {
    membership({ role: "org_member" });
    mockFeed.listOrgConnectPlaces.mockResolvedValue({
      places: [{ cc_place_id: "p1", cv_org_id: null }],
      total: 1,
      linked: true,
    });
  ...
  it("returns 502 when the Connect API is unavailable", async () => {
    membership({ role: "org_member" });
    mockFeed.listOrgConnectPlaces.mockRejectedValue(
