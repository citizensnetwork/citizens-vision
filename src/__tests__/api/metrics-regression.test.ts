...
const { GET } = await import("@/app/api/metrics/regression/route");

function makeRequest(params: Record<string, string>) {
  const url = new URL("http://localhost/api/metrics/regression");
...
describe("GET /api/metrics/regression", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 200 with regression data", async () => {
    const res = await GET(
      makeRequest({
        org_id: VALID_ORG,
        granularity: "week",
